...
static inline uint64_t translate_fnptr(dart_task_action_t fnptr,
...
static dart_ret_t flush_buffer_all(dart_amsgq_t amsgq, bool blocking);

dart_ret_t dart_amsg_init() {
  dart_ret_t res;
  ...
  msgq_msgsize =
      dart__base__env__size(DART_AMSGQ_MSGSIZE_ENVSTR, DEFAULT_MSGCACHE_SIZE);
...
dart_ret_t dart_amsg_openq(size_t msg_size, size_t msg_count, dart_team_t team,
                           dart_amsgq_t *queue) {
  dart_ret_t ret;
  size_t team_size;
  ...
  ret = amsgq_impl.openq(msgq_msgsize,
...
dart_ret_t dart_amsg_trysend(dart_team_unit_t target, dart_amsgq_t amsgq,
                             dart_task_action_t fn, const void *data,
                             size_t data_size) {
...
dart_ret_t dart_amsg_bcast(dart_team_t team, dart_amsgq_t amsgq,
                           dart_task_action_t fn, const void *data,
                           size_t data_size) {
  size_t size;
...
dart_ret_t dart_amsg_buffered_send(dart_team_unit_t target, dart_amsgq_t amsgq,
                                   dart_task_action_t fn, const void *data,
                                   size_t data_size) {
  ...
  while ((pos + size_required) > msgq_msgsize) {
    ...
    if (amsgq->flush_info != NULL) {
      if (DART_OK != flush_buffer_all(amsgq, false)) {
        // either we or another thread is currently flushing -> send as single
        // message NOTE: flush_buffer_all may call process() internally so this
        // may be recursive
        dart_ret_t ret;
      ...
      if (cache->pos + size_required > msgq_msgsize) {
        ...
        do {
          ...
          ret = amsgq_impl.trysend(target, amsgq->impl, cache->buffer,
                                   cache->pos);
  ...
  DART_LOG_TRACE("Cached message: fn=%p, r=%d, ds=%d, id=%d", msg->header.fn,
                 msg->header.remote.id, msg->header.data_size,
                 msg->header.msgid);
  pthread_rwlock_unlock(&cache->mutex);
  return DART_OK;
}
...
static dart_ret_t flush_buffer_all(dart_amsgq_t amsgq, bool blocking) {
  ...
  if (DART_OK != dart__base__mutex_trylock(&amsgq->mutex))
    return DART_PENDING;
...
dart_ret_t flush_buffer(dart_amsgq_t amsgq, bool blocking) {
  if (amsgq->flush_info != NULL) {
    ...
    if (cache != NULL) {
      if (cache->pos > 0 || blocking) {
        ...
        do {
          ...
          if (DART_ERR_AGAIN == ret) {
            // TODO: this has the potential to deadlock as a message may trigger
            // outgoing messages!
            amsgq_impl.process(amsgq->impl);
          } else if (ret != DART_OK) {
            DART_LOG_ERROR("Failed to flush message cache!");
...
dart_ret_t dart_amsg_flush_buffer(dart_amsgq_t amsgq) {
  flush_buffer(amsgq, false);
}

dart_ret_t dart_amsg_process(dart_amsgq_t amsgq) {
  return amsgq_impl.process(amsgq->impl);
}

dart_ret_t dart_amsg_process_blocking(dart_amsgq_t amsgq, dart_team_t team) {
  size_t size;
  dart_team_size(team, &size);
  if (size == 1) {
...
dart_ret_t dart_amsg_closeq(dart_amsgq_t amsgq) {
...
static inline uint64_t translate_fnptr(dart_task_action_t fnptr,
  ...
  if (needs_translation) {
    ...
    DART_LOG_TRACE("Translated function pointer %p into %p on unit %i", fnptr,
                   (void *)remote_fnptr, global_target_id.id);
  }
  return remote_fnptr;
}
...
static inline dart_ret_t exchange_fnoffsets() {
  ...
  if (MPI_Allgather(&base, 1, MPI_UINT64_T, bases, 1, MPI_UINT64_T,
 ...
 */
void dart__amsgq__process_buffer(void *dbuf, size_t tailpos) {
  ...
  while (pos < tailpos) {
    ...
    DART_LOG_TRACE("Invoking active message %p id=%d from %i on data %p of "
                   ...
                   header->fn, header->msgid, header->remote.id, data,
                   header->data_size, startpos);
    header->fn(data);
    num_msg++;
  }
}
