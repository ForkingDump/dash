 ...
 */
#define DART_AMSGQ_SQ_SLEEP_ENVSTR "DART_AMSGQ_SQ_SLEEP"
...
static dart_ret_t dart_amsg_sq_openq(size_t msg_size, size_t msg_count,
                                         dart_team_t team,
  ...
  if (sleeptime.tv_sec == -1) {
    sleep_us = dart__base__env__us(DART_AMSGQ_SQ_SLEEP_ENVSTR, 0);
  ...
  res->queues = calloc(res->comm_size, sizeof(SlotQueue *));
  for (int i = 0; i < res->comm_size; ++i) {
    res->queues[i] = slot_queue_init(msg_count, i, res->comm);
  }
...
static dart_ret_t dart_amsg_sq_sendbuf(dart_team_unit_t target,
                                           struct dart_amsgq_impl_data *amsgq,
                                           const void *data, size_t data_size) {
  DART_LOG_DEBUG("dart_amsg_trysend: u:%i ds:%zu", target.id, data_size);
  if (slot_queue_enqueue(amsgq->queues[target.id], data, data_size)) {
...
static dart_ret_t
amsg_sq_process_internal(struct dart_amsgq_impl_data *amsgq,
                             bool blocking) {
  int64_t tailpos;
...
static dart_ret_t dart_amsg_sq_process(struct dart_amsgq_impl_data *amsgq) {
  return amsg_sq_process_internal(amsgq, false);
}

static dart_ret_t
dart_amsg_sq_process_blocking(struct dart_amsgq_impl_data *amsgq,
                                  dart_team_t team) {
  int flag = 0;
  MPI_Request req;
  ...
  do {
    amsg_sq_process_internal(amsgq, false);
  ...
  amsg_sq_process_internal(amsgq, true);
  MPI_Barrier(amsgq->comm);
  return DART_OK;
}

static dart_ret_t dart_amsg_sq_closeq(struct dart_amsgq_impl_data *amsgq) {
  ...
  for (int i = 0; i < amsgq->comm_size; ++i) {
    slot_queue_destroy(amsgq->queues[i]);
  }
  free(amsgq->queues);
...
dart_ret_t dart_amsg_sq_init(dart_amsgq_impl_t *impl) {
  impl->openq = dart_amsg_sq_openq;
  impl->closeq = dart_amsg_sq_closeq;
  impl->trysend = dart_amsg_sq_sendbuf;
  impl->process = dart_amsg_sq_process;
  impl->process_blocking = dart_amsg_sq_process_blocking;
  return DART_OK;
}
