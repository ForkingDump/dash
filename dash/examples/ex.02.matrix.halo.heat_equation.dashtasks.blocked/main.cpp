...
using MatrixT =
    dash::Matrix<double, 2, typename PatternT::index_type, PatternT>;
...
struct locked_cout_t {
  ...
  template <typename T> locked_cout_t &operator<<(const T &value) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    cerr << "Not enough arguments ./<prog> matrix_ext iterations chunk_size"
         << endl;
    return 1;
  }
  ...
  PatternT pattern(dash::SizeSpec<2>(matrix_ext, matrix_ext), dist, tspec,
                   dash::Team::All());
  MatrixT src_matrix(pattern);
  MatrixT dst_matrix(pattern);
  // Stencil points for North, South, West, and East, Center is defined
  // automatically
  StencilSpecT stencil_spec(StencilT(-1, 0), StencilT(1, 0), StencilT(0, -1),
                            StencilT(0, 1));
  ...
  DEBUGOUT << "pattern.local_extent(0): " << pattern.local_extent(0)
           << std::endl;
  const std::map<int, std::vector<long>> halo_boundary_offsets = {
      {1,
       {0, 1,
        pattern.local_extent(1) - 1}}, // N depends on the row and both corners
      {3,
       {0, 1,
        pattern.local_extent(0) -
            1}}, // W depends on the column and both corners
      {5,
       {0, 1,
        pattern.local_extent(0) -
            1}}, // E depends on the column and both corners
      {7,
       {0, 1,
        pattern.local_extent(1) - 1}}, // S depends on the row and both corners
  };
  ...
  constexpr const std::array<dash::halo::RegionPos, 2> region_positions(
      {dash::halo::RegionPos::PRE, dash::halo::RegionPos::POST});

  auto ptr_matrix = src_matrix.lbegin();
  auto ptr_matrix2 = dst_matrix.lbegin();
  if (myid == 0) {
    for (auto i = 0; i < src_matrix.local.extent(0); ++i) {
      for (auto j = 0; j < src_matrix.local.extent(1);
           ++j, ++ptr_matrix, ++ptr_matrix2) {
        if (i < 100 && j < 100) {
  ...
  */
  ...
  for (auto iter = 0; iter < iterations; ++iter) {
      ...
      if (region_ptr != nullptr) {
        ...
        dash::tasks::async(
            ...
            [&](auto deps) {
              // local halo memory
              deps = dash::tasks::out(
                  *src_op_ptr->halo_memory().range_at(idx).first);
    ...
    while (begin_y <= coords_last[0]) {
      ...
      while (begin_x <= coords_last[1]) {
        ...
        dash::tasks::async(
            "UPDATE_INNER",
            [=]() {
              src_op_ptr->inner.update_blocked(
                  {begin_y, begin_x}, {end_y, end_x}, dst_matrix_lbegin,
                  [&, iter, begin_x, begin_y](auto *center, auto *center_dst,
                                              auto offset,
                                              const auto &offsets) {
                    double dtheta = (center[offsets[0]] + center[offsets[1]] -
                                     2 * *center) /
                                        (dx * dx) +
                                    (center[offsets[2]] + center[offsets[3]] -
                                     2 * *center) /
                                        (dy * dy);
                    *center_dst = *center + k * dtheta * dt;

                    DEBUGOUT << "{" << iter << "} Computing value at " << offset
                             << "{" << begin_y << ", " << begin_x << "} : {"
                             << *center << " ";
                    for (int i = 0; i <= 3; ++i) {
                    ...
                    for (int i = 0; i <= 3; ++i) {
                      if (center[offsets[i]] != 1.0) {
                        DEBUGOUT << "value in dimension " << i
                                 << " not one: " << center[offsets[i]]
            ...
            [&](auto deps) {
              ...
              if (begin_x == coords_begin[1]) {
                ...
                // TODO: check that dimension is correct
                deps =
                    dash::tasks::in(*src_op_ptr->boundary.iterator_at(3).first);
              } else {
                // dependency to left inner block
                deps = dash::tasks::in(
                    src_op_ptr->inner.at({begin_y, begin_x - chunk_size}));
              }
              ...
              if (end_x == coords_last[1]) {
                ...
                // TODO: check that dimension is correct
                deps =
                    dash::tasks::in(*src_op_ptr->boundary.iterator_at(5).first);
                auto coords =
                    src_op_ptr->boundary.iterator_at(5).first.coords();
                DEBUGOUT << "Block at {" << begin_y << ", " << begin_x
                         << "} depends on boundary 5 {" << coords[0] << ", "
                         << coords[1] << "}" << std::endl;
              } else {
              ...
              if (begin_y == coords_begin[0]) {
                ...
                // TODO: check that dimension is correct
                deps =
                    dash::tasks::in(*src_op_ptr->boundary.iterator_at(1).first);
              } else {
                // dependency to upper inner block
                deps = dash::tasks::in(
                    src_op_ptr->inner.at({begin_y - chunk_size, begin_x}));
              }
              ...
              if (end_y == coords_last[0]) {
                ...
                // TODO: check that dimension is correct
                deps =
                    dash::tasks::in(*src_op_ptr->boundary.iterator_at(7).first);
              } else {
    ...
    for (int idx = 0; idx < max_idx; ++idx) {
      ...
      if (idx == 4)
        continue;
      DEBUGOUT << "Creating boundary update task for region " << idx << " in "
               << iter << std::endl;
      dash::tasks::async(
          "UPDATE_BOUNDARY",
          [&, dst_matrix_lbegin, src_op_ptr, iter, idx]() {
            ...
            src_op_ptr->boundary.update(
                ...
                [=](auto &it) {
                  ...
                  DEBUGOUT << "{" << iter
                           << "} Computing BOUNDARY value in region " << idx
                           << " at " << it.lpos() << ": { " << core << " ";
                  for (int i = 0; i <= 3; ++i) {
          ...
          [&](auto deps) {
            ...
            auto blockpos = 0;
            // the position of the first element in the last block in this
            // dimension
            auto end = coords_begin[dim] +
                       ((coords_last[dim] - coords_begin[dim]) / chunk_size) *
                           chunk_size;
            DEBUGOUT << "dim " << dim << ", rpos " << rpos << ", end " << end
                     << ", coords_end[dim] " << coords_last[dim]
                     << ", coords_begin[dim] " << coords_begin[dim]
                     << std::endl;
            while (blockpos <= coords_last[dim]) {
              // for left/upper boundary: walk down with second index 0
              if (dash::halo::RegionPos::PRE == rpos) {
                deps = dash::tasks::in(src_op_ptr->inner.at(
                    {(dim == 0) ? blockpos : 0, (dim == 1) ? blockpos : 0}));
              }
              // for right/lower boundary: walk down with second index pointing
              // to last block
