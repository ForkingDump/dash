...
using matrix_t =
    dash::Matrix<double, 2, typename pattern_t::index_type, pattern_t>;
...
int main(int argc, char *argv[]) {
  ...
  for (auto d = 0; d < iterations; ++d) {
    ...
    dash::tasks::async(
        ...
        dash::tasks::out(*new_halo), dash::tasks::out(*current_halo));
    ...
    dash::tasks::taskloop(
        ...
        [&, current_op](auto begin, auto end) {
          current_op->inner.update(
              begin, end, new_matrix.lbegin(),
              [&](auto *center, auto *center_dst, auto offset,
                  const auto &offsets) {
                // std::cout << "INNER " << dash::distance(begin, end) <<
                // std::endl;
                double dtheta =
                    (center[offsets[0]] + center[offsets[1]] - 2 * *center) /
                        (dx * dx) +
                    (center[offsets[2]] + center[offsets[3]] - 2 * *center) /
                        (dy * dy);
        ...
        [&](auto begin, auto end, auto deps) {
          ...
          deps = dash::tasks::in(*new_halo);
          // NOTE: no OUT dependency here because we synchronize with the dummy
          // task above
        });
    ...
    for (int dim = 0; dim < 2; ++dim) {
      {
        auto it_pair =
            current_op->boundary.iterator_at(dim, dash::halo::RegionPos::PRE);
        auto new_it_pair =
            new_op->boundary.iterator_at(dim, dash::halo::RegionPos::PRE);
        auto idx =
            dash::halo::RegionCoords<2>::index(dim, dash::halo::RegionPos::PRE);
        auto region_ptr = current_halo->halo_block().halo_region(idx);
        ...
        if (region_ptr != nullptr) {
          ...
          dash::tasks::async(
              "UPDATE_BOUNDARY",
              [&, lbegin, it_pair, current_op, idx]() {
                current_op->boundary.update(
                    it_pair.first, it_pair.second, lbegin, [&](auto &it) {
                      auto core = *it;
                      double dtheta =
                          (it.value_at(0) + it.value_at(1) - 2 * core) /
                              (dx * dx) +
                          (it.value_at(2) + it.value_at(3) - 2 * core) /
                              (dy * dy);
              ...
              // dummy dependency to synchronize with next iteration
              dash::tasks::in(*new_halo), dash::tasks::out(*new_it_pair.first));
        }
      }
      {
        auto it_pair =
            current_op->boundary.iterator_at(dim, dash::halo::RegionPos::POST);
        auto new_it_pair =
            new_op->boundary.iterator_at(dim, dash::halo::RegionPos::POST);
        auto idx = dash::halo::RegionCoords<2>::index(
            dim, dash::halo::RegionPos::POST);
        auto region_ptr = current_halo->halo_block().halo_region(idx);
